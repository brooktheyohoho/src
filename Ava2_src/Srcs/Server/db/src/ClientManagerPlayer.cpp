...
bool CreateItemTableFromRes(MYSQL_RES * res, std::vector<TPlayerItem> * pVec, DWORD dwPID)
{
	...
	for (int i = 0; i < rows; ++i)
	{
		...
		for (int j = 0; j < GLOVE_ATTR_MAX_NUM; j++)
		{
			str_to_number(item.randomAttr[j].bType, row[cur++]);
			str_to_number(item.randomAttr[j].sValue, row[cur++]);
		}
#endif
...
/*
			...
			iLen += snprintf(szQuery+iLen, sizeof(szQuery)-iLen," FROM player.item WHERE owner_id=%d AND (window < %d or window = %d",pTab->id, SAFEBOX, DRAGON_SOUL_INVENTORY);
#ifdef ENABLE_SPECIAL_STORAGE
			iLen += snprintf(szQuery+iLen, sizeof(szQuery)-iLen," or window = %d or window = %d or window = %d or window = %d", UPGRADE_INVENTORY, BOOK_INVENTORY, STONE_INVENTORY, CHANGE_INVENTORY);
#endif
#ifdef ENABLE_SWITCHBOT
			iLen += snprintf(szQuery+iLen, sizeof(szQuery)-iLen," or window = %d", SWITCHBOT);
#endif
#ifdef CHANGE_EQUIP_WORLDARD
			iLen += snprintf(szQuery+iLen, sizeof(szQuery)-iLen," or window = %d", CHANGE_EQUIP);
#endif
...
#ifdef ENABLE_GLOVE_SYSTEM
					",randomtype0,randomvalue0,randomtype1,randomvalue1,randomtype2,randomvalue2,randomtype3,randomvalue3,randomtype4,randomvalue4,randomtype5,randomvalue5,randomtype6,randomvalue6 "
#endif
#ifdef ENABLE_NEW_NAME_ITEM
					",name "
#endif
...
#ifdef ENABLE_SPECIAL_STORAGE
					" or window = %d or window = %d or window = %d or window = %d or window = %d"
#endif
#ifdef ENABLE_SWITCHBOT
					" or window = %d"
#endif
...
#ifdef ENABLE_SPECIAL_STORAGE
					, UPGRADE_INVENTORY, BOOK_INVENTORY, STONE_INVENTORY, CHANGE_INVENTORY , COSTUME_INVENTORY
#endif
#ifdef ENABLE_SWITCHBOT
					,SWITCHBOT
#endif
...
/*
		int iLen = snprintf(queryStr, sizeof(queryStr),"SELECT id,window+0,pos,count,vnum");
#ifdef ELEMENT_SPELL_WORLDARD
		iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen,",grade_elements");
		for(BYTE j=1;j<4;++j)
			iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen,",attackelement%d",j);
		iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen,",elementstype");
		for(BYTE j=1;j<4;++j)
			iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen,",elementsvalue%d",j);
#endif
		for(BYTE j=0;j<ITEM_SOCKET_MAX_NUM;++j)
			iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen,",socket%d",j);
		for(BYTE j=0;j<ITEM_ATTRIBUTE_MAX_NUM;++j)
			iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen,",attrtype%d,attrvalue%d",j,j);
#ifdef ENABLE_GLOVE_SYSTEM
		for(BYTE j=0;j<GLOVE_ATTR_MAX_NUM;++j)
			iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen,",randomtype%d,randomvalue%d",j,j);
#endif
#ifdef ENABLE_NEW_NAME_ITEM
		iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen,",name");
#endif
		
		iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen," FROM player.item WHERE owner_id=%d AND (window < %d or window = %d",pTab->id, SAFEBOX, DRAGON_SOUL_INVENTORY);
#ifdef ENABLE_SPECIAL_STORAGE
		iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen," or window = %d or window = %d or window = %d or window = %d", UPGRADE_INVENTORY, BOOK_INVENTORY, STONE_INVENTORY, CHANGE_INVENTORY);
#endif
#ifdef ENABLE_SWITCHBOT
		iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen," or window = %d", SWITCHBOT);
#endif
#ifdef CHANGE_EQUIP_WORLDARD
		iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen," or window = %d", CHANGE_EQUIP);
#endif
		iLen += snprintf(queryStr+iLen, sizeof(queryStr)-iLen,")");
*/
...
#ifdef ENABLE_BATTLE_PASS
	if (row[col])
		thecore_memcpy(pkTab->battlePassData, row[col], sizeof(pkTab->battlePassData));
	else
...
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		GetTablePostfix(), pi->player_id, SAFEBOX, DRAGON_SOUL_INVENTORY, UPGRADE_INVENTORY, BOOK_INVENTORY, STONE_INVENTORY, CHANGE_INVENTORY, COSTUME_INVENTORY, SWITCHBOT, CHANGE_EQUIP);
#else
		...
		GetTablePostfix(), pi->player_id, SAFEBOX, DRAGON_SOUL_INVENTORY, UPGRADE_INVENTORY, BOOK_INVENTORY, STONE_INVENTORY, CHANGE_INVENTORY, COSTUME_INVENTORY, SWITCHBOT);
#endif
#else
#ifdef CHANGE_EQUIP_WORLDARD
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM item%s WHERE owner_id=%d AND (window < %d or window = %d or window = %d or window = %d or window = %d or window = %d or window = %d or window = %d)", 
		GetTablePostfix(), pi->player_id, SAFEBOX, DRAGON_SOUL_INVENTORY, UPGRADE_INVENTORY, BOOK_INVENTORY, STONE_INVENTORY, CHANGE_INVENTORY, COSTUME_INVENTORY, CHANGE_EQUIP);
#else
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM item%s WHERE owner_id=%d AND (window < %d or window = %d or window = %d or window = %d or window = %d or window = %d or window = %d)", 
		GetTablePostfix(), pi->player_id, SAFEBOX, DRAGON_SOUL_INVENTORY, UPGRADE_INVENTORY, BOOK_INVENTORY, STONE_INVENTORY, COSTUME_INVENTORY, CHANGE_INVENTORY);
#endif
#endif
#else
#ifdef ENABLE_SWITCHBOT //Replace your old line with the whole ifdef/else statement
