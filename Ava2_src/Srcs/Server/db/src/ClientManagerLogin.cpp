...
void CClientManager::RESULT_LOGIN_BY_KEY(CPeer * peer, SQLMsg * msg)
{
	...
	if (g_stLocale == "gb2312")
	{
		snprintf(szQuery, sizeof(szQuery),			
				"SELECT id, name, job, level, alignment, st, ht, dx, iq, part_main, part_hair, part_sash, x, y, skill_group, change_name, conquerorlevel, sungma_str, sungma_hp, sungma_move, sungma_inmune FROM player%s WHERE account_id=%u",
				GetTablePostfix(), info->pAccountTable->id);
	}
	else
	{
		snprintf(szQuery, sizeof(szQuery),
				"SELECT id, name, job, level, playtime, st, ht, dx, iq, part_main, part_hair, part_sash, x, y, skill_group, change_name, conquerorlevel, sungma_str, sungma_hp, sungma_move, sungma_inmune FROM player%s WHERE account_id=%u",
				GetTablePostfix(), info->pAccountTable->id);
	}
#else
	if (g_stLocale == "gb2312")
