...
bool CClientManager::InitializeTables()
{
	...
	if (!InitializeMultiRefineTable())
	{
		sys_err("InitializeMultiRefineTable FAILED");
		return false;
	}
#endif
...
#ifdef ENABLE_MULTI_REFINE_WORLDARD
bool CClientManager::InitializeMultiRefineTable()
{
	char query[2048];

	snprintf(query, sizeof(query), "SELECT id, vnum, refine_vnum, refine_set FROM refine_multi_evolution%s", GetTablePostfix());
	...
	if (m_pMultiRefineTable)
	{
		sys_log(0, "RELOAD: refine_multi_evolution");
		delete [] m_pMultiRefineTable;
		m_pMultiRefineTable = NULL;
	}

	m_iMultiRefineTableSize = pRes->uiNumRows;

	m_pMultiRefineTable  = new TMultiRefineTable[m_iMultiRefineTableSize];
	memset(m_pMultiRefineTable, 0, sizeof(TMultiRefineTable) * m_iMultiRefineTableSize);

	TMultiRefineTable* prt = m_pMultiRefineTable;
	MYSQL_ROW data;
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		str_to_number(prt->refine_vnum, data[col++]);
		str_to_number(prt->refine_set, data[col++]);
	
		sys_log(0, "MULTI REFINE: vnum %d refine_vnum %d refine_set %d ", prt->vnum, prt->refine_vnum, prt->refine_set);
		prt++;
	}
	return true;
}
...
bool CClientManager::InitializeItemAttrTable()
{
	char query[4096];
		...
		snprintf(query, sizeof(query),
			"SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear, pendant, glove, costume_body, costume_hair, costume_weapon FROM item_attr%s ORDER BY apply",
			GetTablePostfix());

#else
		snprintf(query, sizeof(query),
			"SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear, pendant, costume_body, costume_hair, costume_weapon FROM item_attr%s ORDER BY apply",
			GetTablePostfix());
#endif
...
#ifdef ENABLE_GLOVE_SYSTEM
		snprintf(query, sizeof(query),
			"SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear, pendant, glove FROM item_attr%s ORDER BY apply",
			GetTablePostfix());
