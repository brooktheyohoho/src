...
{
	"<Bank> Some character is not allowed.",
	"<Bank> Password is incorrect, need to have 4 characters.",
	"<Bank> Account %s already exist!",
	"<Bank> Account %s created with succes ! Password: %s.",
	
	"<Bank> Account %s what you try to send money, not exist!",
	"<Bank> The amount introduced is so less.",
	"<Bank> You do not own this money in the bank.",
	"<Bank> Target info succes send: [Account name: %s , Money: %d]",

	"<Bank> The amount introduced for add is less like one.",
	"<Bank> You have too little Yang!",
	"<Bank> The amount introduced for withdraw is less like one.",
	"<Bank> You do not own this money in the bank.",
	"<Bank> You can not hold so much yang in inventory.",

	"<Bank> Password is incorrect! Need to have 4 characters.",
	"<Bank> Repeat password is incorrect! Need to have 4 characters.",
	"<Bank> Password is not the same with repeat password.",
	"<Bank> Repeat password is not the same with first password.",
	"<Bank> You already have this password, please put other password.",
	"<Bank> Your password has been successfully changed! New password is: %s",

	"<Bank> You cannot do that again so fast, wait %d second!"
};
...
{	
	...
	std::auto_ptr<SQLMsg> pDuration(DBManager::instance().DirectQuery(buf));		
...
bool BankManager::Send_Money(LPCHARACTER ch, const char* mAccount, const char* mPassword, const char* mTargetAccount, const char* mAmount)
{
	...
	std::auto_ptr<SQLMsg> ret(DBManager::instance().DirectQuery(buf));
	...
	std::auto_ptr<SQLMsg> mex(DBManager::instance().DirectQuery(targetBuf));
