...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsNewPet() && !pkChr->IsMountSystem() && pkChr->GetRider() == NULL)
#else
...
ACMD(do_purge)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Yardımcını gondermeden bu i?lemi yapamazsın. ");
		return;
	}
#endif
	char arg1[256];
...
ACMD(do_reset_time_dungeon)
{
	...
	if (CDungeonInfoExtern::instance().GetTimeRespawnDungeonActual(ch,CDungeonInfoExtern::instance().GetIndexMapDungeon(id_dungeon)) > 0){
		...
		snprintf(dungeon_time_respawn, sizeof(dungeon_time_respawn), "dungeon_info.dungeon_time_respawn_%d", CDungeonInfoExtern::instance().GetIndexMapDungeon(id_dungeon));
...
ACMD(do_set_rarity)
{
	...
	if (vecArgs.size() < 4) { 
		ch->ChatPacket(CHAT_TYPE_INFO, "WRONG COMMAND!");
		ch->ChatPacket(CHAT_TYPE_INFO, "/set_rarity wearName percent value");
		ch->ChatPacket(CHAT_TYPE_INFO, "/set_rarity wearName value value");
		ch->ChatPacket(CHAT_TYPE_INFO, "/set_rarity wearName type value");
		ch->ChatPacket(CHAT_TYPE_INFO, "wear names - weapon, body, head, neck, ear, foots, wrist, ring0, ring1");
		return; 
	}
	std::map<std::string, BYTE> m_wearIndex = {
		{"weapon",WEAR_WEAPON},
		{"body",WEAR_BODY},
		{"head",WEAR_HEAD},
		{"neck",WEAR_NECK},
	...
	auto it = m_wearIndex.find(vecArgs[1].c_str());
	if (it != m_wearIndex.end())
	{
		if (vecArgs[2] == "percent")
		{
			LPITEM item = ch->GetWear(it->second);
			if (item)
			{
				long rarityValue;
				str_to_number(rarityValue, vecArgs[3].c_str());
				if (rarityValue < 0)
					rarityValue = 0;
				long rarityMaxValue = item->GetRarityMaxValue();
				long newRarityValue = long(float(rarityValue) * (float(rarityMaxValue) / 100.0));
				item->ModifyPoints(false);
				item->SetSocket(RARITY_VALUE_INDEX, newRarityValue);
				item->ModifyPoints(true);
				ch->ChatPacket(CHAT_TYPE_INFO, "Set Rarity is successfuly");
			}
			else
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "%s don't have in equip slot!",it->first.c_str());
			}

		}
		else if (vecArgs[2] == "type")
		{
			LPITEM item = ch->GetWear(it->second);
			if (item)
			{
				long rarityType;
				str_to_number(rarityType, vecArgs[3].c_str());
				if (rarityType < 0)
					rarityType = 0;
				item->ModifyPoints(false);
				item->SetSocket(RARITY_TYPE_INDEX, rarityType);
				item->ModifyPoints(true);
				ch->ChatPacket(CHAT_TYPE_INFO, "Set Rarity is successfuly");
			}
			else
		...
		else if (vecArgs[2] == "value")
		{
			...
			if (item)
			{
				...
				if (rarityValue < 0)
					rarityValue = 0;
				item->ModifyPoints(false);
				item->SetSocket(RARITY_VALUE_INDEX, rarityValue);
				item->ModifyPoints(true);
				ch->ChatPacket(CHAT_TYPE_INFO, "Set Rarity is successfuly");
			}
			else
