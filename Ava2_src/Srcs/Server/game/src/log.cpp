...
{
	char szQuery[512];
	snprintf(szQuery, sizeof(szQuery), "INSERT INTO log.exchanges (type, playerA, playerB, goldA, goldB, x, y, date) VALUES (%d, %lu, %lu, %d, %d, %ld, %ld, NOW())", type, dwPID1, dwPID2, goldA, goldB, x, y);
	SQLMsg * pkMsg(DBManager::instance().DirectQuery(szQuery));
	SQLResult * pRes = pkMsg->Get();

	if (!pkMsg || pkMsg->Get()->uiAffectedRows == 0 || pkMsg->Get()->uiAffectedRows == (my_ulonglong)-1) {
