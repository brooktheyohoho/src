...
teleport_area:
	...
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushboolean(L, (ch->GetExchange() || ch->GetShop() || ch->IsOpenSafebox() || ch->IsCubeOpen()));
		return 1;
	}
	...
	ALUA(pc_open_shop0)
	{
		...
		if (ch->IsOpenSafebox() || ch->GetExchange() || ch->GetShop() || ch->IsCubeOpen())
		{
	...
	ALUA(pc_set_conqueror_level)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "918");
			return 0;
		}
			
		if (ch->GetLevel() < gPlayerMaxLevel)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "919");
			return 0;
		}

		ch->PointChange(POINT_CONQUEROR_LEVEL, 1);
		return 1;
	}
