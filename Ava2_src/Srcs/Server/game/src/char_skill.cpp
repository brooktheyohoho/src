...
void SetPolyVarForAttack(LPCHARACTER ch, CSkillProto * pkSk, LPITEM pkWeapon)
{
	if (ch->IsPC())
	{
		...
		{
#if defined(ENABLE_RARITY)
			long maxWep = pkWeapon->GetValue(4);
			if (pkWeapon->GetSocket(RARITY_VALUE_INDEX) > 0)
			{
				int rarityMaxWep = GetRarityPoint(-2, pkWeapon->GetSocket(RARITY_TYPE_INDEX)) * (float(maxWep)/100.0);
				maxWep += rarityMaxWep;
			}
			int iWep = number(pkWeapon->GetValue(3), maxWep);
#else
...
#if defined(ENABLE_RARITY)
			long maxMtk = pkWeapon->GetValue(2);
			if(pkWeapon->GetSocket(RARITY_VALUE_INDEX) > 0)
			{
				int rarityMaxMtk = GetRarityPoint(-1, pkWeapon->GetSocket(RARITY_TYPE_INDEX)) * (float(maxMtk)/100.0);
				maxMtk += rarityMaxMtk;
			}
			int iMtk = number(pkWeapon->GetValue(1), maxMtk);
#else
...
#if defined(ENABLE_RARITY) && !defined(CLOSE_ATTACK_DEFENSE_DECREASE_VALUE)
	SetRarityValue(10, true, true);
#endif
...
#ifdef ENABLE_WOLFMAN_CHARACTER
	if(pkVictim && GetParty() && (dwVnum == 94 || dwVnum == 95 || dwVnum == 96 || dwVnum == 110 || dwVnum == 111))
	{
