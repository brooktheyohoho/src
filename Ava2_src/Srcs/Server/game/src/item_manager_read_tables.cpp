...
bool ITEM_MANAGER::ReadCommonDropItemFile(const char * c_pszFileName)
{
	...
	DBManager::Instance().DirectQuery("DELETE from common.common_drop_item;");
#endif
	while (fgets(buf, 1024, fp))
	{
		++lines;
		...
		for (int i = 0; i <= MOB_RANK_S_KNIGHT; ++i)
		{
			...
			if (d[i].iLvStart == 0)
				continue;
#ifdef SHOW_DROPS_IN_DB
			DBManager::Instance().DirectQuery("INSERT INTO common.common_drop_item (vnum, count, lv_start, lv_end, percent) VALUES(%d,%d,%d,%d,%d);", dwItemVnum, 1, d[i].iLvStart, d[i].iLvEnd, dwPct/100);
#endif
...
bool ITEM_MANAGER::ReadMonsterDropItemGroup(const char * c_pszFileName)
{
	...
	if (!loader.Load(c_pszFileName))
		return false;
#ifdef SHOW_DROPS_IN_DB
	DBManager::Instance().DirectQuery("DELETE from common.mob_drop_item;");
#endif
	for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		std::string stName("");
...
#ifdef SHOW_DROPS_IN_DB
					DBManager::Instance().DirectQuery("INSERT INTO common.mob_drop_item (group_name, type, mob, kill_drop, vnum, count, iPartPct, iRarePct) VALUES('%s','%s',%d,%d,%d,%d,%d,%d);", stName.c_str(), strType.c_str(), iMobVnum, iKillDrop, dwVnum, iCount, iPartPct, iRarePct);
#endif
...
#ifdef SHOW_DROPS_IN_DB
					DBManager::Instance().DirectQuery("INSERT INTO common.mob_drop_item (group_name, type, mob, kill_drop, vnum, count, iPartPct) VALUES('%s','%s',%d,%d,%d,%d,%d);", stName.c_str(), strType.c_str(), iMobVnum, iKillDrop, dwVnum, iCount, dwPct);
#endif
		...
		else if ( strType == "limit" )
		{
			...
			for ( int k=1; k < 256; k++ )
			{
				...
				if ( loader.GetTokenVector(buf, &pTok) )
				{
					...
					sys_log(0,"        name %s pct %d count %d", name.c_str(), dwPct, iCount);
#ifdef SHOW_DROPS_IN_DB
					DBManager::Instance().DirectQuery("INSERT INTO common.mob_drop_item (group_name, type, mob, kill_drop, level_limit, vnum, count, iPartPct) VALUES('%s','%s',%d,%d,%d,%d,%d,%d);", stName.c_str(), strType.c_str(), iMobVnum, iKillDrop, iLevelLimit, dwItemVnum, iCount, dwPct);
#endif
