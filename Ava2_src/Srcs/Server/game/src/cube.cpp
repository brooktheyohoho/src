...
void Cube_open (LPCHARACTER ch)
{
	...
	if (ch->IsCubeOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "626");
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "627");
		return;
	}
...
bool Cube_make (LPCHARACTER ch)
{
	...
	if (!(ch)->IsCubeOpen())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, "628");
		return false;
	}
	...
	if (NULL == cube_proto)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "629");
		return false;
	}
	int cube_gold = cube_proto->gold;
	if (cube_gold < 0 || ch->GetGold() < cube_gold)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "630");	// 이 텍스트는 이미 널리 쓰이는거라 추가번역 필요 없음
		return false;
	}
	...
	{
		// 실패
		ch->ChatPacket(CHAT_TYPE_INFO, "631");	// 2012.11.12 새로 추가된 메세지 (locale_string.txt 에 추가해야 함)
...

{
	...
	if ((inven_type == INVENTORY && INVENTORY_MAX_NUM<=inven_index) || 
		...
		(inven_type == COSTUME_INVENTORY && SPECIAL_INVENTORY_MAX_NUM<=inven_index) ||
...
void Cube_request_result_list(LPCHARACTER ch)
{
	...
	//ch->ChatPacket(CHAT_TYPE_INFO, "cube r_list test %d", resultText.length());
	...
	if (resultText.length() == 0)
	{
		...
		//ch->ChatPacket(CHAT_TYPE_INFO, "cube r_list test1 %d %d ", resultList.size(), resultText.size());
		...
		//ch->ChatPacket(CHAT_TYPE_INFO, "cube r_list test2 %d %d %d", resultText.size() , resultText.size() - 20, CHAT_MAX_LEN);

		if (resultText.size() - 20 >= CHAT_MAX_LEN && resultCount > 2)
		{
	...
	//ch->ChatPacket(CHAT_TYPE_INFO, "cube r_list %d %d %s", npcVNUM, resultCount, resultText.c_str());
