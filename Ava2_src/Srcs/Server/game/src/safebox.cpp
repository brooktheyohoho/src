...
bool CSafebox::Add(DWORD dwPos, LPITEM pkItem)
{
	...
	thecore_memcpy(pack.randomAttr, pkItem->GetRandomAttributes(), sizeof(pack.randomAttr));
#endif
...
bool CSafebox::MoveItem(BYTE bCell, BYTE bDestCell, DWORD count)
{
...

	{
		...
		if(item2 != NULL)
		{
			if (item != item2 && item2->IsStackable() && !IS_SET(item2->GetAntiFlag(), ITEM_ANTIFLAG_STACK) && item2->GetVnum() == item->GetVnum()) // 합칠 수 있는 아이템의 경우
			{
