...
#ifdef ENABLE_FISH_GAME
	if (quest::CQuestManager::instance().GetEventFlag("fishgame_event") == 1)
		ms = number(3000, 7000);//decrease lucky.
#endif
...
void Take(fishing_event_info* info, LPCHARACTER ch)
{
	...
	if (info->step == 1)	// 고기가 걸린 상태면..
	{
		...
		switch (ret)
		{
			...
			case -3: // 난이도 때문에 실패
			case -1: // 시간 확률 때문에 실패
				//ch->ChatPacket(CHAT_TYPE_INFO, "668");
				{
					int map_idx = ch->GetMapIndex();
					int prob_idx = GetProbIndexByMapIndex(map_idx);
			...
			case 0:
				//ch->ChatPacket(CHAT_TYPE_INFO, "669 %s", fish_info[info->fish_id].name);
				if (item_vnum)
				{
					FishingSuccess(ch);
	...
	else
	{
		...
		switch (fish_info[idx].used_table[u_index])
		{
			...
			case USED_WATER_STONE:	// 2
				ch->ChatPacket(CHAT_TYPE_INFO, "675");
				break;

			case USED_SHELLFISH:	// 1
				ch->ChatPacket(CHAT_TYPE_INFO, "676");
				ch->AutoGiveItem(SHELLFISH_VNUM);
				break;

			case USED_EARTHWARM:	// 4
				ch->ChatPacket(CHAT_TYPE_INFO, "677");
