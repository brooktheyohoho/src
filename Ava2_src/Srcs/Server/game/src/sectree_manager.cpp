...
struct FPurgeMonsters
{
	void operator() (LPENTITY ent)
	{
		if ( ent->IsType(ENTITY_CHARACTER) == true )
		{
			...
			if (lpChar->IsMonster() == true && !lpChar->IsPet() && !lpChar->IsMountSystem() && !lpChar->IsNewPet())
#else
...
#ifdef ENABLE_MOUNT_SYSTEM
			if (lpChar->IsNPC() == true && !lpChar->IsPet() && !lpChar->IsMountSystem() && !lpChar->IsNewPet())
#else
