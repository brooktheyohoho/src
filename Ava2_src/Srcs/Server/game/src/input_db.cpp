...
void CInputDB::Boot(const char* data)
{
	 ...
	 * MULTI REFINE RECIPE
	 */
	if (decode_2bytes(data) != sizeof(TMultiRefineTable))
	{
		sys_err("multi refine table size error");
		thecore_shutdown();
		return;
	}
	data += 2;
	...
	size = decode_2bytes(data);
	data += 2;
	sys_log(0, "BOOT: MULTI REFINE: %d", size);

	if (size)
	{
				
		if (!CMultiRefineManager::instance().Initialize((TMultiRefineTable*) data, size))
		{
			sys_err("cannot initialize multi refine table");
			thecore_shutdown();
			return;
		}

		data += size * sizeof(TMultiRefineTable);
	}
#endif
...
void CInputDB::SafeboxLoad(LPDESC d, const char * c_pData)
{
	...
	if (ch->GetExchange() || ch->GetShop() || ch->IsCubeOpen()
...
void CInputDB::ItemLoad(LPDESC d, const char * c_pData)
{
	...
	for (DWORD i = 0; i < dwCount; ++i, ++p)
	{
		...
		item->SetRandomAttributes(p->randomAttr);
#endif
...
{
	char notice[256];
	snprintf(notice, sizeof(notice), "731 %s", EMPIRE_NAME(info->bEmpire), info->szName);
	SendNotice(notice);
}
