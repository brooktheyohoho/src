...
{
	// ifstream inf(filename); //TODO Replace this with an file stream ? MartPwnS 28.05.2014
	LoadF lf;
...

	{
		...
		for (it = state_arg_script_map.begin(); it != state_arg_script_map.end(); ++it)
		{
			...
			if (firstElement.find('.') == firstElement.npos)
			{
				// one like login
			...
			else
			{
				...
				path = OUTPUT_FOLDER "/" + firstElement.substr(0, i) + "/" + s.substr(i + 1, s.npos) + "/";
