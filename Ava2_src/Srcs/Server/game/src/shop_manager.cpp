...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	if (pkChr->IsOpenSafebox() || pkChr->GetExchange() || pkChr->GetShop() || pkChr->IsCubeOpen()
...
void CShopManager::Sell(LPCHARACTER ch, BYTE bInv, BYTE bCell, DWORD bCount)
#else
...
#endif
{
	...
	*/
...
#ifdef ENABLE_SPECIAL_STORAGE
	LPITEM item = ch->GetItem(TItemPos(bInv, bCell));
#else
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "931");
		return;
	}
