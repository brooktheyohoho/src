...
enum ECombatZoneSettings
{
	...
	COMBAT_ZONE_SHOP_MAX_LIMIT_POINTS = 1000, // The maximum limit of points that is admitted per day, every evening at 00:00, resets to 0;
	...
	COMBAT_ZONE_MIN_LEVEL = 90, // Minimum attendance level.
	...
	COMBAT_ZONE_WAIT_TIME_TO_PARTICIPATE =  60*2,  //10 * 60, // Waiting time to be able to participate again - 10 minutes;
	...
	COMBAT_ZONE_WAIT_TIME_KILL_AGAIN_PLAYER = 1 * 60, // You have not received any points from this player, not passed the 5 minutes since his last killing - 5 Minutes;

	COMBAT_ZONE_ADD_POINTS_NORMAL_KILLING = 10, // When you kill a player regular;
...
enum ECombatZoneGeneral
{
	...
	COMBAT_ZONE_DIVIDE_NUM_POINTS = 5,
	COMBAT_ZONE_ADD_DEATHS_POINTS = 1,
	COMBAT_ZONE_MAX_ARGS = 4,

	COMBAT_ZONE_VNUM_MOB = 5004,
	COMBAT_ZONE_TIME_BONUS = 60*3,
	COMBAT_ZONE_TIME_SET = 60*60*1,
...
class CCombatZoneManager : public singleton<CCombatZoneManager>
{
	public :
		...
		void CalculatePointsByKiller(LPCHARACTER ch, LPCHARACTER pkVictim ,bool isAttachedTarget);
		...
		void AffectMobKill(LPCHARACTER ch, int vnum_mob);

		bool GetDaySpecific(DWORD day_actual);

		void 	RemoveAffectMob(LPCHARACTER ch, DWORD type);
