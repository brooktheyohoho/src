...
bool COXEventManager::CheckAnswer(bool answer)
{
	...
	std::auto_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("SELECT item_vnum, item_count FROM player.event_items"));
	...
	if (m_vect_Items.size() > 0)
	{
		for (std::map<DWORD, DWORD>::iterator it = m_map_attender.begin(); it != m_map_attender.end(); ++it)
		{
			...
			if (pkChar)
			{
				...
				LPITEM pkItem = ITEM_MANAGER::instance().CreateItem(sItem.dwVnum, sItem.dwCount);
				if (pkItem == NULL) continue;

				int cell;
				if (pkItem->IsDragonSoul())
				{
					cell = pkChar->GetEmptyDragonSoulInventory(pkItem);
					if (cell != -1) pkItem->AddToCharacter(pkChar, TItemPos(DRAGON_SOUL_INVENTORY, cell));
				}
				else
				{
					cell = pkChar->GetEmptyInventory(pkItem->GetSize());
					if(cell != -1) pkItem->AddToCharacter(pkChar, TItemPos(INVENTORY, cell));
				}
			}
		}
	}
