...
#endif
{
	...
	{
		if (pProto->aApplies[i].bType == APPLY_RANDOM)
		{
			item->AddRandomAttribute(pProto->aApplies[i].lValue, item->GetRefineLevel());
		}
	}
#endif
...
#ifdef ENABLE_GLOVE_SYSTEM
	thecore_memcpy(t.randomAttr, item->GetRandomAttributes(), sizeof(t.randomAttr));
#endif
...
//			return;
//
//		const static DWORD dwVnum = CATCH_KING_DROP_ITEM;	// Königskarte
//
//		if ((item = CreateItem(dwVnum, 1, 0, true)))
//			vec_item.push_back(item);
//	}
