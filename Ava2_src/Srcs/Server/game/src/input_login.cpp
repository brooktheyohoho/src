...
/*
	...
	DWORD slot_special_1 = 0;
	DWORD slot_special_2 = 0;
	DWORD slot_special_3 = 0;

	std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT inventory,inventory_special_1,inventory_special_2,inventory_special_3 FROM player.unlock_inventory WHERE pid = %u limit 1;", ch->GetPlayerID()));
	if (pMsg->Get()->uiNumRows > 0){
		MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
		str_to_number(slot, row[0]);
		str_to_number(slot_special_1, row[1]);
		str_to_number(slot_special_2, row[2]);
		str_to_number(slot_special_3, row[3]);
	}
	ch->SetUnlockSlots(slot,slot_special_1,slot_special_2,slot_special_3);
	ch->ChatPacket(CHAT_TYPE_COMMAND, "ManagerInventoryUnlock Hide|%d", slot);
#endif
*/
...
#ifdef ENABLE_MULTI_REFINE_WORLDARD
	CMultiRefineManager::instance().SetInformationRefineMulti(ch,MULTIREFINE_SEND_DATES_CLEAR);
	CMultiRefineManager::instance().SetInformationRefineMulti(ch,MULTIREFINE_SEND_DATES);
#endif
...
//#endif
	...
	if (ch->IsConquerorMap(ch->GetMapIndex()) && ch->IsPC())
	{
		...
		const BYTE bINMUNE = SECTREE_MANAGER::instance().GetSungmaValueAffectByRegion(ch->GetMapIndex(), AFFECT_SUNGMA_INMUNE);
